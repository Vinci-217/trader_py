...
print('='*80)
print('Aggressive激进型策略 2020-2025年回测')
print('='*80)
...
for year in years:
    ...
    for code, name in stocks:
        key = f'{name}_{year}'
        try:
            result = run_backtest(code, name, start, end, strategy_name=f'{name.lower()}_aggressive')
            results[key] = result
...
for name in ['NVDA', 'TSLA', 'AAPL', 'MSFT', 'AMZN', 'GOOGL', 'META', 'AMAT']:
    ...
    print(f'{name}: 6年累计={total_return*100-100:.1f}%, 年化≈{cagr:.1f}%, 最大回撤≈{max_dd:.1f}%, 总交易{trades}次')
